...
void getParamsFromTxt(std::vector<std::array<double, 26>>& params, int& NWorld, int& NReplicats)
{
    int i = 0, j = 0;
    ...
    for(i=0; i<NWorld; i++)
    {
        std::array<double, 26> paramOneWorld = {0,};
...
void runSimu(void)
{
    ...
    std::vector<std::array<double, 26>> params;
    ...
    for(i=0; i<NWorld; i++)
    {
        for(j=0; j<NReplicats; j++)
        {
            ...
            params[i][20], params[i][21], params[i][22], params[i][23], params[i][24], params[i][25]);
